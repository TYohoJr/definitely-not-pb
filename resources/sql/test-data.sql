...
INSERT INTO account_info(app_user_id, account_type_id, email, is_email_confirmed) SELECT au.id,aty.id,'tyohojr@gmail.com',TRUE FROM app_user au, account_type aty WHERE au.username='tyohojr' AND aty.type='premium';

INSERT INTO album(name,app_user_id) SELECT 'testing album',au.id FROM app_user au WHERE au.username='testuser';

INSERT INTO photo(app_user_id,name,description,s3_bucket,s3_key,file_type_id,size) SELECT au.id,'bulbasaur.png','','definitely-not-photobucket','users/1/photos/bulbasaur.png',ft.id,314265 FROM file_type ft, app_user au WHERE ft.type='image/png' AND au.username='testuser';
INSERT INTO photo(app_user_id,name,description,s3_bucket,s3_key,file_type_id,size) SELECT au.id,'flaming_cat-wallpaper.jpg','','definitely-not-photobucket','users/1/photos/flaming_cat-wallpaper-3840x2160.jpg',ft.id,2303876 FROM file_type ft, app_user au WHERE ft.type='image/jpeg' AND au.username='testuser';
INSERT INTO photo(app_user_id,name,description,s3_bucket,s3_key,file_type_id,size) SELECT au.id,'wide-wallpaper.jpg','','definitely-not-photobucket','users/1/photos/fXIoa0N.jpg',ft.id,422396 FROM file_type ft, app_user au WHERE ft.type='image/jpeg' AND au.username='testuser';
